import{a as r,j as E}from"./vendor-animation-DgkgYiNt.js";import{C as F,L as D,S as L,O as G,W,a as H,V as z,P as j,M as k}from"./vendor-three-CB0D37DJ.js";const K=({text:v="Aary.Hinge"})=>{const a=r.useRef(null),p=r.useRef(null),m=r.useRef(null),x=r.useRef(null),f=r.useRef({x:.5,y:.5}),g=r.useRef({x:.5,y:.5}),s=r.useRef(null),b=r.useRef(!0),[S,y]=r.useState(!1);r.useEffect(()=>{new FontFace("Kisthe","url(/Fonts/Kisthe-BF69030b51234d0.otf)").load().then(e=>{document.fonts.add(e),y(!0)}).catch(()=>{y(!0)})},[]);const h=r.useCallback((t,e,o)=>{const u=document.createElement("canvas"),i=u.getContext("2d"),n=Math.min(window.devicePixelRatio,2);u.width=e*n,u.height=o*n,i.scale(n,n),i.clearRect(0,0,e,o);const c=Math.min(e*.18,200);i.font=`${c}px "Kisthe", "Space Grotesk", system-ui, sans-serif`,i.textAlign="center",i.textBaseline="middle";const l=i.createLinearGradient(0,o*.3,e,o*.7);l.addColorStop(0,"#7dd3fc"),l.addColorStop(.5,"#f0abfc"),l.addColorStop(1,"#bef264"),i.fillStyle=l,i.fillText(t,e/2,o/2);const d=new F(u);return d.minFilter=D,d.magFilter=D,d.needsUpdate=!0,d},[]),T=`
  ...
  `,w=r.useCallback(t=>{if(!a.current)return;const e=a.current.getBoundingClientRect();g.current={x:(t.clientX-e.left)/e.width,y:1-(t.clientY-e.top)/e.height}},[]),R=r.useCallback(()=>{if(!p.current||!m.current||!a.current)return;const t=a.current.clientWidth,e=a.current.clientHeight;p.current.setSize(t,e),m.current.uniforms.uResolution.value.set(t,e),s.current&&s.current.dispose(),s.current=h(v,t,e),m.current.uniforms.uTexture.value=s.current},[v,h]);return r.useEffect(()=>{if(!a.current||!S)return;const t=a.current,e=t.clientWidth,o=t.clientHeight,u=new L,i=new G(-1,1,1,-1,0,1),n=new W({antialias:!0,alpha:!0,premultipliedAlpha:!1,powerPreference:"high-performance"});n.setPixelRatio(Math.min(window.devicePixelRatio,2)),n.setSize(e,o),n.setClearColor(0,0),t.appendChild(n.domElement),p.current=n,s.current=h(v,e,o);const c=new H({vertexShader:T,fragmentShader:C,uniforms:{uTexture:{value:s.current},uTime:{value:0},uMouse:{value:new z(.5,.5)},uResolution:{value:new z(e,o)},uPixelSize:{value:8},uDistortionStrength:{value:1}},transparent:!0,depthTest:!1,depthWrite:!1});m.current=c;const l=new j(2,2),d=new k(l,c);u.add(d);const A=Date.now(),M=()=>{x.current=requestAnimationFrame(M),b.current&&(f.current.x+=(g.current.x-f.current.x)*.1,f.current.y+=(g.current.y-f.current.y)*.1,c.uniforms.uTime.value=(Date.now()-A)*.001,c.uniforms.uMouse.value.set(f.current.x,f.current.y),n.render(u,i))};M();const P=new IntersectionObserver(([U])=>{b.current=U.isIntersecting},{threshold:.05});return P.observe(t),window.addEventListener("mousemove",w,{passive:!0}),window.addEventListener("resize",R,{passive:!0}),()=>{P.disconnect(),x.current&&cancelAnimationFrame(x.current),window.removeEventListener("mousemove",w),window.removeEventListener("resize",R),l.dispose(),c.dispose(),s.current&&s.current.dispose(),n.dispose(),t&&n.domElement.parentNode===t&&t.removeChild(n.domElement)}},[v,S,h,w,R,T,C]),E.jsx("div",{ref:a,className:"w-full h-64 md:h-80 lg:h-96 relative",style:{minHeight:"280px"},"aria-label":v,role:"heading","aria-level":"1"})};export{K as default};
